...
function show_tools(id, has_info, has_viewing, has_store) {
    ...
    if (tool_id != null) {
        ...
        if (id.indexOf('dataset') != -1 && id.indexOf('lineage') != -1) {
            if (id.indexOf('mini') == -1)
                dojo.byId(tool_id).style.left = move_Pos1 + move_right + "px";
            else
                dojo.byId(tool_id).style.left = move_Pos0 + "px";
        }

        //lineage model  
        if (id.indexOf('model') != -1 && id.indexOf('lineage') != -1) {
            if (id.indexOf('mini') == -1)
                dojo.byId(tool_id).style.left = move_Pos2 + move_right + "px";
            else
                dojo.byId(tool_id).style.left = move_Pos1b + "px";
        }

        //detail 
        if (dojo.byId('lineage_model_0') == null && dojo.byId('usage_model_0') == null)
            dojo.byId(tool_id).style.left = "651px";
        else if (id.indexOf('detail') != -1 && !lineage_hidden)
            dojo.byId(tool_id).style.left = move_Pos5 + move_right + "px";
        else if (id.indexOf('detail') != -1 && lineage_hidden)
            dojo.byId(tool_id).style.left = move_Pos2b + move_right + "px";

        //usage dataset + lin not hidden
        if (id.indexOf('dataset') != -1 && id.indexOf('usage') != -1 && !lineage_hidden) {
            if (id.indexOf('mini') == -1)
                dojo.byId(tool_id).style.left = move_Pos10 + move_right + "px";
            else
                dojo.byId(tool_id).style.left = move_Pos8 + "px";
        }
        ...
        if (id.indexOf('model') != -1 && id.indexOf('usage') != -1 && !lineage_hidden) {
            if (id.indexOf('mini') == -1)
                dojo.byId(tool_id).style.left = move_Pos7 + move_right + "px";
            else
                dojo.byId(tool_id).style.left = move_Pos6 + "px";
        }

        //usage model + lin hidden
        if (id.indexOf('model') != -1 && id.indexOf('usage') != -1 && lineage_hidden) {
            if (id.indexOf('mini') == -1)
                dojo.byId(tool_id).style.left = move_Pos4 + "px";
            else
                dojo.byId(tool_id).style.left = move_Pos4 + "px";
        }

        if (id.indexOf('mini') == -1)
            dojo.byId(tool_id).style.top = dojo.byId(id).offsetTop + 30 + "px";
        else
            dojo.byId(tool_id).style.top = dojo.byId(id).offsetTop + 40 + "px";
    }
}
...
function show_view_menu() {
    var data;
    mStore.fetchItemByIdentity({identity: "mapping_ids_uuids", onItem: function(item, request) {
            data = item;
        }});

    if (actual_id.indexOf('mini') != -1)
        actual_id = actual_id.replace('_mini', '');
...
function show_info_menu() {
    ...
    var map;
    mStore.fetchItemByIdentity({identity: mapped_id, onItem: function(item, request) {
            map = item;
        }});
...
function focus_element(id, isInBool) { //isInBool determines if dataset is 100% in gn - otherwise we just give a search link
    ...
    //id = mappingID / get mappings, search for mappingID and get real ID
    var mappings;
    mStore.fetchItemByIdentity({identity: "mapping_ids_uuids", onItem: function(item, request) {
            mappings = item;
        }});
    var mappedID = mStore.getValues(mappings, id);
    var detail;
    mStore.fetchItemByIdentity({identity: mappedID, onItem: function(item, request) {
            detail = item;
        }});
    ...
    if (isInBool == true) {                     //open url for given id
        if (typeof detail.gnID == "undefined" || detail.gnID == null)
        ...
        if (detail.gnID != "undefined" && detail.gnID != null) {
            a.href = idDomain + detail.gnID;
            a.innerHTML = "show information";
        } else {
