...
function positionModel(id) {
    ...
    if (num_us_ds > num_lin_ds)
        count = num_us_ds; //get highest amount of datasets to set the top value of lineage model - proper visualization depends on it
    else
        count = num_lin_ds;
...
function positionDatasets() {
    ...
    for (var j = 0; j < 3; j++) { //combinations of min/normal lineage/usage
        ...
        if (j == 0)
            ...
            h_ds = 90;         //normal cards

        if (j == 0 || j == 1)
            num_ds = num_lin_ds; //lineage
        else
            num_ds = num_us_ds;                   //usage
        ...
        if (num_ds % 2 == 0) {
            for (var i = 0; i < num_ds; i++) {
                ...
                if (faktor < 0) {
                    ...
                    if (dojo.byId(id_string + i) != null)
                        dojo.byId(id_string + i).style.top = y_pos_absolute + 'px';
                } else {
                    y_pos_absolute = mod_middle + y_pos_relative;
                    if (dojo.byId(id_string + i) != null)
                        dojo.byId(id_string + i).style.top = y_pos_absolute + 'px';
    ...
    var models;
    mStore.fetchItemByIdentity({identity: "usage_models", onItem: function(item, request) {
            models = item;
        }});
    ...
    for (var k = 0; k < models.usage_model_ids.length && models.usage_model_ids.length > 0; k++) {
        var mapping_JSON;
        mStore.fetchItemByIdentity({identity: "mod_ds_relations", onItem: function(item, request) {
                mapping_JSON = item;
            }});
...
function setWhiteSize() {
    ...
    if (dojo.byId('footer') != null)
        dojo.byId('footer').style.top = (factor * card_size_with_buffer + 980) + "px";
...
function setModels() {
    for (var i = 0; i < num_us_mod; i++) {
        ...
        dojo.style("usage_model_mini_" + i, "margin-top", "5px");  //fix for misplaced mini card
...
var lastID = "";
//shows inputs from models while mouse click //triggered from card_creation / generateCard() - mousedown
function showInputs(id) {
    ...
    for (var i = 0; i < inputLength; i++) {
        if (usage_inputs[id][i] != savedDS) {
            mStore.fetchItemByIdentity({identity: usage_inputs[id][i], onItem: function(item, request) {
                    ds = item;
                }});
            if (ds != null) {
                var is_time = false;
                ...
                var title = ds.title;
                if (title[0].length > 25)
                    title = title[0].substring(0, 25) + "...";
