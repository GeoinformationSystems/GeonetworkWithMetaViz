...
function setLines() {
    ...
    if (lineage_hidden == false) {//lineage in full size
        lineage_ds_left = 255;
        lineage_model_left = 290;
        lineage_model_width = 235;
        detail_left = 590;
    ...
    if (first == false && dojo.byId("actual_lines").hasChildNodes())
        dojo.byId('actual_lines').removeChild(dojo.byId('actual_lines').firstChild);
    else {
        first = false;
    }
    ...
    if (maxInput > num_lin_ds && maxInput > num_us_ds)
        ...
        surface_height = num_us_ds * card_size_with_buffer;
