...
function initData() {
    var ds_data;
    mStore.fetchItemByIdentity({identity: "datasets", onItem: function(item, request) {
            ds_data = item;
        }});
    ...
    var mod_data;
    mStore.fetchItemByIdentity({identity: "models", onItem: function(item, request) {
            mod_data = item;
        }});
    ...
    for (var i = 0; i < num_us_mod; i++) {
        ...
        if (model != null) {
            ...
            if (maxInput < inputs.length)
                maxInput = inputs.length;
        }
    }
    ...
    for (var i = 0; i < num_lin_ds; i++) {
        ...
        if (ds != null) {
            ...
            if ((ds.view != null && ds.view[0] != "") && ((ds.view[0].indexOf("catalog")) < 0) && (ds.view[0].indexOf("null") > 0)) {
                has_view = true;
            }
            if (ds.save != null && ds.save[0] != "") {
                has_save = true;
            }
            if (ds.info != null && ds.info[0] != "") {
                has_info = true;
            }
    ...
    for (var i = 0; i < num_us_ds; i++) {
        ...
        if (ds != null) {
            if (ds.linked_2_modelInput == 1)
                linked_ds.push("usage_dataset_" + i); //saves usage ds which are linked to model
    ...
    var detail;
    mStore.fetchItemByIdentity({identity: detailId.detail_0[0], onItem: function(item, request) {
            detail = item;
        }});
    ...
    if (detail.time != "") {
        ds_time = ",<br />" + detail.time;
        is_time = true;
    }
    if ((detail.view != null && detail.view[0] != "") && ((detail.view[0].indexOf("catalog")) < 0) && (detail.view[0].indexOf("null") > 0)) {
        has_view = true;
    }
    if (detail.save != "") {
        has_save = true;
...
function preprocessData(ds_ids) {
    for (var i = 0; i < ds_ids.length; i++) {
        ...
        if (ds_ids[i] != "paramName") {
            mStore.fetchItemByIdentity({identity: ds_ids[i], onItem: function(item, request) {
                ds = item;
            }});
        
            if (ds != null && ds.paramName != null) {
                if (ds.type == "lineage" || ds.linked_2_modelInput == 1)
                    num_lin_ds++; //changed for proper input display - deleted else because linked=usage
                if (ds.type == "usage")
                    num_us_ds++;
            }
        }
...
function preprocessModel(mod_ids) {
    for (var i = 0; i < mod_ids.length; i++) {
        ...
        if (mod_ids[i] != "paramName") {
            mStore.fetchItemByIdentity({identity: mod_ids[i], onItem: function(item, request) {
                    mod = item;
                }});
            if (mod.type == "lineage")
                num_lin_mod++;
            else if (mod.type == "usage")
                num_us_mod++;
        }
    }
}
...
function initStylesAndTools() {
    ...
    toggleLineage();
    if (num_us_mod > 2)
        setModels(); //to add n-models in card-position 
    setLines();
}