...
function generateCard(type, title, organisation, extent_time, description, view_type, is_vector, is_time_aware, has_info, has_viewing, has_store) {
    ...
    if (view_type == "lineage_dataset")
        html_card += "<div id=\"" + "lineage_dataset_" + lineage_dataset_cards + "\" style=\"top:" + (lineage_dataset_cards - 1) * card_size_with_buffer + "px;\" class=\"description_card\" onmouseover=\"show_tools(\'" + "lineage_dataset_" + lineage_dataset_cards + "\', \'" + has_info + "\',\'" + has_viewing + "\',\'" + has_store + "\')\" onmousedown=\"focus_element(\'" + "lineage_dataset_" + lineage_dataset_cards + "\',false)\">";
    else if (view_type == "usage_dataset")
        html_card += "<div id=\"" + "usage_dataset_" + usage_dataset_cards + "\" style=\"top:" + (usage_dataset_cards - 1) * card_size_with_buffer + "px;\"  class=\"description_card\" onmouseover=\"show_tools(\'" + "usage_dataset_" + usage_dataset_cards + "\', \'" + has_info + "\',\'" + has_viewing + "\',\'" + has_store + "\')\" onmousedown=\"focus_element(\'" + "usage_dataset_" + usage_dataset_cards + "\',true)\">";
    else if (view_type == "lineage_model")
        html_card += "<div id=\"" + "lineage_model_" + lineage_model_cards + "\" class=\"description_card_petrol\" onmouseover=\"show_tools(\'" + "lineage_model_" + lineage_model_cards + "\', \'" + has_info + "\',\'" + has_viewing + "\',\'" + has_store + "\')\">";
    else if (view_type == "usage_model")
        html_card += "<div id=\"" + "usage_model_" + usage_model_cards + "\" class=\"description_card_petrol\" onmouseover=\"show_tools(\'" + "usage_model_" + usage_model_cards + "\', \'" + has_info + "\',\'" + has_viewing + "\',\'" + has_store + "\')\" onmousedown=\"showInputs(\'" + "usage_model_" + usage_model_cards + "\')\">";
    else if (view_type == "detail")
        html_card += "<div id=\"" + "detail_0" + "\" class=\"description_card_colored_petrol\" onmouseover=\"show_tools(\'detail_0" + "\', \'" + has_info + "\',\'" + has_viewing + "\',\'" + has_store + "\')\" onmousedown=\"focus_element(\'" + "detail_0" + "\',true)\">";
    // getting icons if card doesn't contain model information
    if (view_type.indexOf('model') == -1)
        html_card += generateIconset(false, is_vector, is_time_aware, true);   //icons (true = normal size)   
    else
        html_card += generateIconset(true, is_vector, is_time_aware, true);
...
function generateIconset(is_model, is_vector, is_time_aware, normal_size) {
    ...
    if (normal_size && is_time_aware) {
        html_icons += "<div class=\"icon_time\" id=\"time_" + num_time + "\"></div>";
        num_time++;
    }
    else if (!normal_size && is_time_aware) {
        html_icons += "<div class=\"icon_time\" id=\"time_" + num_time + "\" style=\"top:10px;left:35px;\"></div>";
        num_time++;
    }
...
function generateDescriptionset(type, title, organisation, extent_time, description, view_type) {
    ...
    if (view_type == "lineage_dataset") {
        id = lineage_dataset_cards;
        lineage_dataset_cards++;
    }
    else if (view_type == "usage_dataset") {
        id = usage_dataset_cards;
        usage_dataset_cards++;
    }
    else if (view_type == "lineage_model") {
        id = lineage_model_cards;
        lineage_model_cards++;
    }
    else if (view_type == "usage_model") {
        id = usage_model_cards;
        usage_model_cards++;
    }
    else if (view_type == "detail")
        id = 0;

    if (view_type == "lineage_model" || view_type == "usage_model")
        html_description += "<p id=\"" + view_type + "_" + id + "_type\" class=\"object_type\" style=\"left:10px;\">" + type + "</p>";
    else
        html_description += "<p id=\"" + view_type + "_" + id + "_type\" class=\"object_type\">" + type + "</p>";

    if (title[0].length > 23) {
        var tit;
        if (title[0].length > 50)
            tit = title[0].substring(0, 50) + "...";
        else
            tit = title[0];

        if (view_type == "lineage_model" || view_type == "usage_model")
            html_description += "<p id=\"" + view_type + "_" + id + "_title\" class=\"object_title\" style=\"font-size:12px;left:10px;\">" + tit + "</p>";
        else
            html_description += "<p id=\"" + view_type + "_" + id + "_title\" class=\"object_title\" style=\"font-size:12px;\" onmousedown=\"focus_element(\'" + view_type + "_" + id + "\')\">" + tit + "</p>";
    } else {
        if (view_type == "lineage_model" || view_type == "usage_model")
            html_description += "<p id=\"" + view_type + "_" + id + "_title\" class=\"object_title\" style=\"left:10px;\">" + title + "</p>";
        else
            html_description += "<p id=\"" + view_type + "_" + id + "_title\" class=\"object_title\" onmousedown=\"focus_element(\'" + view_type + "_" + id + "\')\">" + title + "</p>";
    }
    if (organisation != null && organisation[0].length > 36) {
        if (organisation[0].length > 55)
            organisation[0] = organisation[0].substring(0, 50) + "...";

        if (view_type == "lineage_model" || view_type == "usage_model")
            html_description += "<p id=\"" + view_type + "_" + id + "_organisation\" class=\"object_organisation\" style=\"left:10px;\">" + organisation + "</p>";
        else
            html_description += "<p id=\"" + view_type + "_" + id + "_organisation\" class=\"object_organisation\">" + organisation + "</p>";
    } else {
        if (organisation == null || organisation == "") {
            if (view_type == "lineage_model" || view_type == "usage_model")
                html_description += "<p id=\"" + view_type + "_" + id + "_organisation\" class=\"object_organisation\" style=\"left:10px;\">" + "processor not defined" + "</p>";
            else
                html_description += "<p id=\"" + view_type + "_" + id + "_organisation\" class=\"object_organisation\">" + "processor not defined" + "</p>";
        } else {
            if (view_type == "lineage_model" || view_type == "usage_model")
                html_description += "<p id=\"" + view_type + "_" + id + "_organisation\" class=\"object_organisation\" style=\"left:10px;\">" + organisation + "</p>";
            else
...
function generate_mini_card(type, view_type, is_vector, is_time_aware, has_info, has_viewing, has_store) {
    ...
    if (view_type == "lineage_dataset") {
        id = lineage_dataset_mini_cards;
        lineage_dataset_mini_cards++;
    }
    else if (view_type == "usage_dataset") {
        id = usage_dataset_mini_cards;
        usage_dataset_mini_cards++;
    }
    else if (view_type == "lineage_model") {
        id = lineage_model_mini_cards;
        lineage_model_mini_cards++;
    }
    else if (view_type == "usage_model") {
        id = usage_model_mini_cards;
        usage_model_mini_cards++;
    }
    else if (view_type == "detail")
        id = 0;

    if (view_type == "lineage_dataset")
        html_card += "<div id=\"" + "lineage_dataset_mini_" + (lineage_dataset_mini_cards - 1) + "\" class=\"description_card\" style=\"width:65px;height:50px;top:" + ((lineage_dataset_mini_cards - 1) * 70 + 10) + "px;\" onmouseover=\"show_tools(\'" + "lineage_dataset_mini_" + (lineage_dataset_mini_cards - 1) + "\', \'" + has_info + "\',\'" + has_viewing + "\',\'" + has_store + "\')\" onmousedown=\"focus_element(\'" + "lineage_dataset_" + lineage_dataset_cards + "\',false)\">";
    else if (view_type == "usage_dataset")
        html_card += "<div id=\"" + "usage_dataset_mini_" + (usage_dataset_mini_cards - 1) + "\" class=\"description_card\" style=\"width:65px;height:50px;top:" + ((usage_dataset_mini_cards - 1) * 70 + 10) + "px;\" onmouseover=\"show_tools(\'" + "usage_dataset_mini_" + (usage_dataset_mini_cards - 1) + "\', \'" + has_info + "\',\'" + has_viewing + "\',\'" + has_store + "\')\" onmousedown=\"focus_element(\'" + "usage_dataset_" + usage_dataset_cards + "\',true)\">";
    else if (view_type == "lineage_model")
        html_card += "<div id=\"" + "lineage_model_mini_" + (lineage_model_mini_cards - 1) + "\" class=\"description_card_petrol\" style=\"width:65px;height:50px;\" onmouseover=\"show_tools(\'" + "lineage_model_mini_" + (lineage_model_mini_cards - 1) + "\', \'" + has_info + "\',\'" + has_viewing + "\',\'" + has_store + "\')\">";
    else if (view_type == "usage_model")
        html_card += "<div id=\"" + "usage_model_mini_" + (usage_model_mini_cards - 1) + "\" class=\"description_card_petrol\" style=\"width:65px;height:50px;\" onmouseover=\"show_tools(\'" + "usage_model_mini_" + (usage_model_mini_cards - 1) + "\', \'" + has_info + "\',\'" + has_viewing + "\',\'" + has_store + "\')\">";
    else if (view_type == "detail")
        html_card += "<div id=\"" + "detail_0_mini" + "\" class=\"description_card_colored_petrol\" style=\"width:65px;height:50px;\" onmouseover=\"show_tools(\'detail_0_mini" + "\', \'" + has_info + "\',\'" + has_viewing + "\',\'" + has_store + "\')\">";

    if (view_type.indexOf('model') != -1)
        html_card += generateIconset(true, is_vector, is_time_aware, false);   //icons (false = mini size)  
    else
        html_card += generateIconset(false, is_vector, is_time_aware, false);
